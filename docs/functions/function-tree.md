...
| M99 | MSW Mock Layer | msw 内部 mock 边界命名空间（不进 shared 业务树）；职责由 ADR-0012 + docs/msw-architecture.md 承载 | 已废弃|
...
| 子项 ID | 名称 | 类型 | 交付 | 说明 | 状态|
|---|---|---|---|---|---|
| M04.F03.I01 | 授权码签发 | 接口 | 前端+后端 | 校验 session+redirect_uri+scope 后签发一次性 authorization_code；msw 作为 backend mock 服务真实前端 | 已上线|
| M04.F03.I02 | 令牌交换 | 接口 | 前端+后端 | authorization_code grant（`oauth.tsp:15`）；msw handler 与 nextjs/springboot/aspnetcore 四方对齐 | 已上线|
| M04.F03.I03 | 令牌刷新 | 接口 | 前端+后端 | refresh_token grant（`oauth.tsp:15`）；msw rotate 语义对齐 oracle    | 已上线|
...
| M06.F03 | 审计写入助手 | 旧版审计写入 helper；目标 DDL 不再保留 | 已废弃|
...
## M99 MSW Mock Layer（已废弃）

> msw 内部 mock 边界命名空间（不进 shared 业务树）；职责由 ADR-0012 + `docs/msw-architecture.md` 承载。功能清单不再演进。
> I 级子项见 [§已废弃功能子项](#已废弃功能子项)。

| 功能 ID | 功能名称 | 说明 | 状态|
|---|---|---|---|
| M99.F01 | Fixture data consistency | msw 内部 — 跨端 seed + 进程内 Map 状态 | 已废弃|
| M99.F02 | MSW handlers emit OAuth 2.0 server | msw 内部 — handlers emit OAuth 2.0 server（RFC 6749） | 已废弃|
| M99.F03 | 独立 HTTP 服务暴露 | msw 内部 — Express + @mswjs/http-middleware | 已废弃|
| M99.F04 | handler 覆盖范围 / 全局一致性 | msw 内部 — handlers 覆盖 | 已废弃|
| M99.F05 | trace.json fns 集合 | msw 内部 — fnReporter + trace_env | 已废弃|
| M99.F06 | Contract-test live mode | msw 内部 — ADR-0016 实时断言 | 已废弃|
...
> M99（msw 内部 mock layer 命名空间）的 F 级 / I 级子项已统一登记到 [§M99 MSW Mock Layer（已废弃）](#m99-msw-mock-layer已废弃) 段；本表只列业务树 ID（M00/M01/M04 及已废弃 M02/M03/M05/M06/M08/M09）。
